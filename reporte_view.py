"""Proyecto GAMMA - Vista M4: Reportes y Estadísticas"""
...
class ReporteView(QWidget):
    ...
    def _setup_ui(self):
        ...
        t=QLabel("Reportes y Estadísticas"); t.setStyleSheet(titulo_style())
        d=QLabel("Dashboard estadístico del sistema · Módulo M4"); d.setStyleSheet(desc_style())
        col.addWidget(t); col.addWidget(d); enc.addLayout(col); enc.addStretch()
        btn=btn_secondary("↻  Actualizar"); btn.clicked.connect(self._cargar); enc.addWidget(btn)
        layout.addLayout(enc)

        layout.addWidget(BannerWidget("📊","Reportes y Estadísticas del Sistema",
